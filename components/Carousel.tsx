...
const CarouselSizes = async ({categorie} : apiProps) => {
const dataPop = await fetchMovies(`/movie/${categorie}?language=en-US&page=1`)
  return (
    <Carousel
      opts={{
        align: "start",
      ...
      <CarouselContent className="px-10">
        {Array.from({ length: 10 }).map((_, index) => (
          <CarouselItem key={index} className="md:basis-1/3 lg:basis-1/5 ">
            <div className="p-1">
              <Card className="bg-center bg-cover bg-no-repeat" style={{backgroundImage : `url(https://image.tmdb.org/t/p/original/${dataPop.results[index + 1].backdrop_path})`}}>
